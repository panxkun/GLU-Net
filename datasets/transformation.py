...
class SynthPairTnf(object):
    ...
    def __init__(self, use_cuda=True, geometric_model='affine', crop_factor=9 / 16, output_size=(240, 240),
                 padding_factor=0.5, occlusion_factor=0):
    ...
    def __call__(self, batch):
        ...
        if self.occlusion_factor != 0:
            ...
            # apply mask
            cropped_image_batch = torch.mul(cropped_image_batch, 1 - mask_1) + torch.mul(
                cropped_image_batch[rolled_indices_1, :], mask_1)
            warped_image_batch = torch.mul(warped_image_batch, 1 - mask_2) + torch.mul(
                warped_image_batch[rolled_indices_1, :], mask_2)
    ...
    def get_occlusion_mask(self, mask_size, occlusion_factor):
        ...
        box_w = torch.round(
            out_w * torch.sqrt(torch.FloatTensor([occlusion_factor])) * (1 + (torch.rand(b) - 0.5) * 2 / 5))
...
class Theta_gen():
    ...
    def __call__(self):
        ...
        if self.geometric_model == 'affine' or self.geometric_model == 'afftps':
            ...
            lambda: 0.75 ~ 1.25
            '''
            rot_angle = (np.random.rand(1) - 0.5) * 2 * np.pi / 6;  # between -np.pi/12 and np.pi/12
            sh_angle = (np.random.rand(1) - 0.5) * 2 * np.pi / 6;  # between -np.pi/6 and np.pi/6
            lambda_1 = 1 + (2 * np.random.rand(1) - 1) * 0.5;  # between 0.75 and 1.25
            lambda_2 = 1 + (2 * np.random.rand(1) - 1) * 0.5;  # between 0.75 and 1.25
            tx = (2 * np.random.rand(1) - 1) * 0.5;  # between -0.25 and 0.25
            ty = (2 * np.random.rand(1) - 1) * 0.5;
