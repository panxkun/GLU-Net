...
def calculate_epe_and_pck_per_dataset(test_dataloader, network, device, threshold_range, path_to_save=None,
    ...
    threshold_range_pixel = np.linspace(1, 15, num=15)
    pck_threshold_pixel_list = {'v': np.zeros(threshold_range_pixel.shape, dtype=np.float32),
                                'i': np.zeros(threshold_range_pixel.shape, dtype=np.float32)}
    ...
    for i_batch, mini_batch in pbar:
        ...
        for t_id, threshold in enumerate(threshold_range_pixel):
            pck_threshold_pixel_list[seq_name[0]][t_id] += correct_correspondences(flow_est,
                                                                             flow_target,
                                                                             alpha=threshold / img_size,
                                                                             img_size=img_size)
    ...
    output = {'aepe_mean': np.mean(aepe_array),
              'aepe_list':aepe_array,
              'seq_list': name_array,
              'pck_threshold_1': np.mean(pck_thresh_1_over_image),
              'pck_threshold_5': np.mean(pck_thresh_5_over_image),
              'pck_percent_5': np.mean(pck_alpha_0_05_over_image),
              'alpha_threshold': threshold_range.tolist(),
              'pck_per_threshold_over_dataset': np.float32(array_n_correct_correspondences /
                                                           (n_registered_pxs + 1e-6)).tolist(),
              'pck_per_threshold_pixel_average': {'v': pck_threshold_pixel_list['v'].tolist(),
                                                  'i': pck_threshold_pixel_list['i'].tolist()}
            }
